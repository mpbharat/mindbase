...
export interface Env {
  ...
  BRAIN_WORKER_URL: string; // e.g. https://mind-worker.<your-subdomain>.workers.dev
}
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (path === "/" || path === "/health") {
      return json({ status: "ok", service: "mind-worker" });
    }
...
Respond with ONLY a JSON object: {"is_generated": true} or {"is_generated": false}`;
    ...
    try {
      // ─── GET /context ─────────────────────────────────────────────────────
      // Returns a <mind-context> block for Claude agents
