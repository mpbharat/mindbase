 ...
 * Only needs BRAIN_URL and BRAIN_API_KEY — no infra setup required.
 */
...
function fetch(url) {
  return new Promise((resolve, reject) => {
    const get = (u) => https.get(u, { headers: { 'User-Agent': 'brain-join/1.0' } }, (res) => {
...
async function main() {
  ...
  console.log('  · BRAIN_URL  (your Cloudflare Worker URL)');
  console.log('  · BRAIN_API_KEY  (the key you generated during install)\n');

  // ── 1. Check Claude Code CLI ──────────────────────────────────────────────
  if (!hasCommand('claude --version')) {
    fail('Claude Code CLI not found.');
    console.error('  Install it from: https://claude.ai/code');
    console.error('  Then re-run:     npx brain-join\n');
    process.exit(1);
  }
  ok('Claude Code CLI');
  ...
  // ── 3. Fetch brain-install skill (contains the brain-join section) ────────
  ...
  const systemAppend = [
    'BRAIN JOIN MODE.',
    `The brain-install skill is at ${skillPath}.`,
    'Read it and execute ONLY the "Machine 2 — brain-join" section.',
    'Ask the user for their BRAIN_URL and BRAIN_API_KEY,',
    'build brain-cli, set their agent name, add shell exports,',
    'and write brain-sync into ~/.claude/CLAUDE.md.',
    'Start immediately.',
  ].join(' ');

  const result = spawnSync(
    'claude',
    ['--append-system-prompt', systemAppend, 'Run brain-join to connect this machine to my existing Brain.'],
