...
**Announce at start:** "I'm using the brain-install skill. I'll handle everything — Cloudflare setup, database, CLI, project mapping, dashboard, and wiring myself to sync automatically. You just answer a few questions."
...
- A Cloudflare R2 bucket storing files (Brain Drive)
- brain-cli installed locally so Claude can read/write brain from any project
...
cd ~/brain
npm install
cd brain-worker && npm install
cd ../brain-cli && npm install
cd ../brain-dashboard && npm install
```
...
```bash
cp ~/brain/brain-worker/wrangler.toml.example ~/brain/brain-worker/wrangler.toml
```

Now edit `~/brain/brain-worker/wrangler.toml`:
...
Copy the deployed URL — it looks like `https://brain-worker.XXXXXXXX.workers.dev`.

Now update `BRAIN_WORKER_URL` in `wrangler.toml`:
```toml
BRAIN_WORKER_URL = "https://brain-worker.XXXXXXXX.workers.dev"
```
...
### 5c — Brain Worker URL as secret

```bash
echo "https://brain-worker.XXXXXXXX.workers.dev" | npx wrangler secret put BRAIN_WORKER_URL
```
...
```bash
BRAIN_URL="https://brain-worker.XXXXXXXX.workers.dev"
BRAIN_API_KEY="<the-key-you-generated>"
...
curl -s -X POST "$BRAIN_URL/admin/migrate-vectors" \
  -H "Authorization: Bearer $BRAIN_API_KEY"
```
...
```bash
# Brain
export BRAIN_URL="https://brain-worker.XXXXXXXX.workers.dev"
export BRAIN_API_KEY="<the-key>"
export BRAIN_AGENT_NAME="claude-mac:<their-name>"  # e.g. claude-mac:john
```
...
```bash
curl -s -X POST "$BRAIN_URL/project" \
  -H "Authorization: Bearer $BRAIN_API_KEY" \
...
## Phase 9 — Write brain-sync into CLAUDE.md
...
Prefer global (`~/.claude/CLAUDE.md`) so brain-sync applies everywhere.
...
BRAIN_URL="https://brain-worker.XXXXXXXX.workers.dev"
BRAIN_API_KEY="<the-key>"
BRAIN_AGENT_NAME="claude-mac:<their-name>"
...
curl -s -X POST "$BRAIN_URL/agent-state" \
  -H "Authorization: Bearer $BRAIN_API_KEY" \
  -H "Content-Type: application/json" \
  -d "{\"agent_name\":\"$BRAIN_AGENT_NAME\",\"state\":{\"status\":\"idle\",\"current_task\":null,\"next_task\":\"<next goal>\"}}"
```
...
Substitute actual values for `$BRAIN_URL`, `$BRAIN_API_KEY`, `$BRAIN_AGENT_NAME` in the written file — do not write literal `$BRAIN_URL`, write the actual URL string.
...
Create `.env.local`:
```
VITE_BRAIN_URL=https://brain-worker.XXXXXXXX.workers.dev
VITE_BRAIN_API_KEY=<the-key>
```
...
Copy the Pages URL (e.g. `https://brain-dashboard-xxx.pages.dev`).
...
curl -s -X POST "$BRAIN_URL/memory" \
  -H "Authorization: Bearer $BRAIN_API_KEY" \
  -H "Content-Type: application/json" \
  -d "{\"content\":\"Brain install complete on $(hostname) — $(date)\",\"category\":\"fact\",\"importance\":6,\"agent_name\":\"$BRAIN_AGENT_NAME\"}"
...
>
> On a second machine, run `npx brain-join` — it just needs your URL and API key, no infra to set up."

---

## Machine 2 — brain-join (Much Simpler)

When the user says "brain-join" or "set up brain on this machine":

You need two things from them: `BRAIN_URL` and `BRAIN_API_KEY`.
...
```bash
export BRAIN_URL="<existing-url>"
export BRAIN_API_KEY="<existing-key>"
export BRAIN_AGENT_NAME="claude-mac:<this-machine>"
source ~/.zshrc
```

Write brain-sync into `~/.claude/CLAUDE.md` exactly as in Phase 9, but with this machine's agent name.
...
| Memory doesn't appear | Check BRAIN_API_KEY matches what's in wrangler secrets |
| Dashboard blank | Check VITE_BRAIN_URL in `.env.local` matches worker URL |
| R2 upload fails | Confirm bucket name in `wrangler.toml` is `brain-drive` |
...
- Never write the literal string `$BRAIN_API_KEY` into CLAUDE.md — write the actual value
- Never skip Phase 9 — if brain-sync isn't in CLAUDE.md, sessions won't save automatically
