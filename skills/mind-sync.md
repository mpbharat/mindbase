...
**This skill is installed automatically by mindbase-install. You do not add this manually.**

It documents the session lifecycle behavior that `mindbase-install` writes into your `~/.claude/CLAUDE.md`.
...
curl -s -X POST "$MIND_URL/session" \
  -H "Authorization: Bearer $MIND_API_KEY" \
  -H "Content-Type: application/json" \
  -d "{\"agent_name\":\"$MIND_AGENT_NAME\",\"summary\":\"<same summary>\",\"project_id\":<id or null>}"

curl -s -X POST "$MIND_URL/agent-state" \
  -H "Authorization: Bearer $MIND_API_KEY" \
  -H "Content-Type: application/json" \
  -d "{\"agent_name\":\"$MIND_AGENT_NAME\",\"state\":{\"status\":\"idle\",\"current_task\":null,\"next_task\":\"<next session goal>\"}}"
```
